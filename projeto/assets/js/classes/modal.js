var Modal =  new Class({
	...
	create: function(){
		...
		var length = jQuery(".modal").size();
		
		var div = "<div id='"+this.id+"' aria-hidden='true' role='dialog' tabindex='-1' class='modal fade in' data-backdrop='true'>";
		div += "<div class='modal-dialog modal-lg'><div class='modal-content'></div></div></div>";
	...
	execute: function(){
		...
		if( !Default.isEmpty(this.url) ){
			...
			if(this.loading ){
				jQuery(this.selector).find(".modal-content").html("<div class='modal-header'><button type='button' class='close modal-close mt-10' data-dismiss='modal' aria-hidden='true'>&times;</button><h4 class='modal-title'>Carregando...</h4></div><div class='modal-body text-center'><i class='fa fa-spinner fa-spin fa-2x fa-fw'></i></div>");
